...
exports.braodcast_list = async(req, res) =>{
   ...
   try {
      const broadcast_lists = await BroadcastList.find({"user_wabaID": user_wabaID})
...
exports.delete_broadcast_list = async(req, res) => {
   ...
   try {
      ...
      const broadcast_list = await BroadcastList.deleteOne({"user_wabaID": user_wabaID, "title": title});
